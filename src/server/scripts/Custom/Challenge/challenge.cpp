...
void ChallengeMod::ReplaceChallenge(Player* player, uint32 mapId, uint32 challengeLv)
{
	...
	{
		if (player->GetGUID().GetCounter() == itr->guid && mapId == itr->mapId)//GetGUIDLow
		{
	...
	if (!exsist)
	{
		...
		temp.guid = player->GetGUID().GetCounter();//GetGUIDLow
		temp.mapId = mapId;
...
{
	for (auto itr = PlayerChallengeVec.begin(); itr != PlayerChallengeVec.end(); itr++)
		if (player->GetGUID().GetCounter() == itr->guid && mapId == itr->mapId && itr->challengeLv == challengeLv)
			return true;
...
bool ChallengeMod::AddGossipOrTele(Player* player, uint32 triggerId)
{
	...
	{
		if (Player* leader = ObjectAccessor::FindPlayerInOrOutOfWorld(group->GetLeaderGUID().GetCounter()))
		{
	...
	for (auto itr = ChallengeVec.begin(); itr != ChallengeVec.end(); itr++)
		if (mapId == itr->MapId)
		{
			if (MapEntry const* mapEntry = sMapStore.LookupEntry(mapId))
			{
				...
				{
					AddGossipItemFor(player,GOSSIP_ICON_CHAT, itr->GossipText + " - 已绑定该难度", triggerId, itr->Level);
	...
	if (!text.empty())
		AddGossipItemFor(player,GOSSIP_ICON_CHAT, text + " - 重置该难度", triggerId, 10000, "重置" + text + "\n将解除绑定，确定重置吗？", 0, false);
	...
	for (auto itr = ChallengeVec.begin(); itr != ChallengeVec.end(); itr++)
		if (mapId == itr->MapId)
		{
			if (HasChallenge(player, itr->MapId, itr->Level))
				...
				AddGossipItemFor(player,GOSSIP_ICON_CHAT, itr->GossipText, triggerId, itr->Level, sReq->Notice(player, abs(itr->ReqId), "选择", itr->GossipText), sReq->Golds(abs(itr->ReqId)), false);
		}
	...
	void OnGossipSelect(Player* player, uint32 menu_id, uint32 triggerId, uint32 action) override
	{
		...
		if (action == 10000)
		{
			...
			if (Group* group = player->GetGroup())
			{
				for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
					if (Player* member = itr->GetSource())
					{
						...
						sChallengeMod->ResetChallenge(member->GetGUID().GetCounter(), mapId);
					}
			...
			else
			{
				...
				sChallengeMod->ResetChallenge(player->GetGUID().GetCounter(), mapId);
			}
