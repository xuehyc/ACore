...
void HonorRank::Load()
{
	...
	do
	{
		...
		HRUpTemp.title				= fields[0].Get<uint32>();
		HRUpTemp.reqId				= fields[1].Get<uint32>();
		HRUpTemp.rewId				= fields[2].Get<uint32>();
		HRUpTemp.lootRate			= fields[3].Get<float>();
		HRUpTemp.exchangeRate_0		= fields[4].Get<uint32>();
		HRUpTemp.exchangeRate_1		= fields[5].Get<uint32>();
		HRUpTemp.strengthenRate		= fields[6].Get<uint32>();
		HRUpTemp.sigilRate			= fields[7].Get<uint32>();
		HRUpTemp.removeGemRate		= fields[8].Get<uint32>();
...
void HonorRank::AddCurrHRMenu(Player* player)
{
	...
	if (player->HasTitle(1) || player->HasTitle(15))
		AddGossipItemFor(player,GOSSIP_ICON_CHAT, hr_name, SENDER_HR_MENU, 0);
}
...
void HonorRank::AddNextHRMenu(Player* player, Object* obj)
{
	...
	if (title != 0 && !GetGossipText(title).empty())
		AddGossipItemFor(player,GOSSIP_ICON_CHAT, GetGossipText(title), SENDER_HR_UP, title, sReq->Notice(player, GetReqId(title), GetGossipText(title), ""), sReq->Golds(GetReqId(title)), false);
