...
{
	VIPVec.clear();
	QueryResult result1 = WorldDatabase.Query(
	...
	if (result1)
	{
		do
		{
			...
			VIPTemplate VIPTemp;
			VIPTemp.vipLv				= fields[0].Get<uint32>();
			VIPTemp.reqId				= fields[1].Get<uint32>();
			VIPTemp.lootRate			= fields[2].Get<float>();
			VIPTemp.exchangeRate_0		= fields[3].Get<uint32>();
			VIPTemp.exchangeRate_1		= fields[4].Get<uint32>();
			VIPTemp.strengthenRate		= fields[5].Get<uint32>();
			VIPTemp.sigilRate			= fields[6].Get<uint32>();
			VIPTemp.removeGemRate		= fields[7].Get<uint32>();
			VIPTemp.rewId				= fields[8].Get<uint32>();
			VIPTemp.icon				= fields[9].Get<std::string>();
			VIPTemp.name				= fields[10].Get<std::string>();
			VIPTemp.timeRewId			= fields[11].Get<uint32>();
			VIPTemp.gossipText			= fields[12].Get<std::string>();
	...
	for (size_t i = 0; i < len; i++)
	{
		if (player->vipLevel == VIPVec[i].vipLv - 1)
		{
			if (sReq->Check(player, VIPVec[i].reqId))
			{
				...
				LoginDatabase.Execute("UPDATE account SET viplevel = viplevel + 1 WHERE id = '%u'", player->GetSession()->GetAccountId());
                //org->DirectPExecute
				player->vipLevel++;
				...
				//sWorld->SendScreenMessage(msg);//org
                //已在world.h添加相关函数,但是无效
				return;
			}
		}
	}
...
void VIP::AddGossip(Player* player, Object* obj)
{
	...
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, GetGossipText(player), SENDER_VIP_CURR, GOSSIP_ACTION_INFO_DEF);
...
public:
	...
	{
		QueryResult result = LoginDatabase.Query("SELECT vipLevel FROM account WHERE id = '%u'", player->GetSession()->GetAccountId());
		if (!result)
			player->vipLevel = 0;
		else
		{
			Field* fields = result->Fetch();
			player->vipLevel = fields[0].Get<int32>();
		}
	}
};
