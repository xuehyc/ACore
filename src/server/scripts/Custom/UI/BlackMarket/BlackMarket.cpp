...
void BlackMarket::Load()
{
	...
	if (QueryResult result = WorldDatabase.Query("SELECT id,itmemId,itemCount,reqId FROM _ui_blackmarket"))
	{
		do
		{
			...
			Temp.itemId		= fields[1].Get<uint32>();
			Temp.itemCount	= fields[2].Get<uint32>();
