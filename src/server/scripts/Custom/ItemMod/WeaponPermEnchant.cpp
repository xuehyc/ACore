...
void ItemMod::AddWeaponPermList(Player* player, Item* item)
{
	...
	if (undefineEnchant && currEnchantId)
		AddGossipItemFor(player,GOSSIP_ICON_CHAT, ossCurrEnchant.str(), senderValue(PERM_ENCHANTMENT_SLOT, currEnchantId), ACTION_REMOVE_ENCHANT_UNDEFINE, "移除光效", DEFAULT_REMOVE_ENCHANT_GOLDS * GOLD, 0);

	if (!undefineEnchant && currEnchantId)
		for (uint32 i = 0; i < len; i++)
			if (currEnchantId == FilterVec[i].enchantId)
			{
				AddGossipItemFor(player,GOSSIP_ICON_CHAT, ossCurrEnchant.str(), senderValue(PERM_ENCHANTMENT_SLOT, currEnchantId), ACTION_ITEM_REMOVE_ENCHANT, "移除光效" + sCF->GetItemLink(item->GetEntry()), 0, 0);
				break;
			}
	...
	for (uint32 i = player->flag_i; i < len; i++)
	{
		...
		{
			player->rowId = 0;
			AddGossipItemFor(player,GOSSIP_ICON_CHAT, "下页", GOSSIP_SENDER_MAIN, ACTION_WEAPON_PERMENCHANT_NEXT_MENU_SHOW);
			if (player->pageId >= 2)
				AddGossipItemFor(player,GOSSIP_ICON_CHAT, "上页", GOSSIP_SENDER_MAIN, ACTION_PREV_PAGE);
			break;
		}
	}
...
void ItemMod::LookupOrBuyWeaponPermEnchant(Player* player, Item* item, uint32 sender)
{
	...
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "购买", sender, ACTION_WEAPONPERM_BUY, sReq->Notice(player, reqId, sCF->GetItemLink(item->GetEntry()), "附加光效"), sReq->Golds(reqId), 0);
	//HasPermEchant(item) ? AddGossipItemFor(player,GOSSIP_ICON_CHAT, "查看", sender, ACTION_WEAPONPERM_LOOKUP, "将会移除当前武器光效，确定吗？", 0, 0) : 
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "查看", sender, ACTION_WEAPONPERM_LOOKUP);
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "返回", GOSSIP_SENDER_MAIN, ACTION_WEAPON_PERMENCHANT_CURR_MENU_SHOW);
	player->PlayerTalkClass->GetGossipMenu().SetMenuId(MENU_ID);
	SendGossipMenuFor(player,DEFAULT_GOSSIP_MESSAGE, player->GetGUID());
}
...
void ItemMod::LookupWeaponPermEnchant(Player* player, Item* item, uint32 enchantId)
{
	...
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "购买", enchantId, ACTION_WEAPONPERM_BUY, sReq->Notice(player, reqId, sCF->GetItemLink(item->GetEntry()), "附加光效"), sReq->Golds(reqId), 0);
	//HasPermEchant(item) ? AddGossipItemFor(player,GOSSIP_ICON_CHAT, "查看", enchantId, ACTION_WEAPONPERM_LOOKUP, "将会移除当前武器光效，确定吗？", 0, 0) : 
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "查看", enchantId, ACTION_WEAPONPERM_LOOKUP);
	AddGossipItemFor(player,GOSSIP_ICON_CHAT, "返回", GOSSIP_SENDER_MAIN, ACTION_WEAPON_PERMENCHANT_CURR_MENU_SHOW);
	player->PlayerTalkClass->GetGossipMenu().SetMenuId(MENU_ID);
	SendGossipMenuFor(player,DEFAULT_GOSSIP_MESSAGE, player->GetGUID());
