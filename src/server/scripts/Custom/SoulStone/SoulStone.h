...
public:
    ...
    void SetPlayerMsg(Player * player, std::string msg) { if (!player) return; uint32 guid = player->GetGUID().GetCounter(); _SoulStonePlayerMsgMap[guid] = msg; }
    std::string GetPlayerMsg(Player * player) { if (!player) return ""; uint32 guid = player->GetGUID().GetCounter(); return _SoulStonePlayerMsgMap[guid]; }
