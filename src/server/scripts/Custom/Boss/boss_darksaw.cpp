...
public:
	...
	struct boss_darksawAI : public ScriptedAI
	{
		...
		{
			if (victim->GetGUID().GetCounter() == me->GetEntry()) // Wanneer een Clone.
			{
			...
			{
				me->Say("哈...你完蛋了...", LANG_UNIVERSAL, NULL);
		...
		void UpdateAI(uint32 diff)
		{
			...
			if (_has_orb_adds == true && addztimer <= diff)
			{
				...
				if (_current_orbs < ADD_MAX_ADDS)
				{
					...
					Unit* target = NULL; target = SelectTarget(SelectTargetMethod::MaxThreat, 0);
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_PLAYSOUND_CAST1A:
					...
					if (t == 14803) { me->Yell("用身体去感受！", LANG_UNIVERSAL, NULL); }
					if (t == 14771) { me->Yell("绝不留情！", LANG_UNIVERSAL, NULL); }
					if (t == 14751) { me->Yell("你的队伍一无是处！", LANG_UNIVERSAL, NULL); }
				...
				case EVENT_PLAYSOUND_CAST1B:
					...
					if (t == 14795) { me->Yell("那么, 你的勇气何在？", LANG_UNIVERSAL, NULL); }
					if (t == 14786) { me->Yell("无论是生是死，你将永无宁日！", LANG_UNIVERSAL, NULL); }
					if (t == 14787) { me->Yell("痛苦，折磨!", LANG_UNIVERSAL, NULL); }
				...
				case EVENT_RANDOM_CHAT:
					if (_chatter_timer > MIN_PAUSE_CHATTER)
					{
						...
						if (t == 1){
							...
							if (t == 14738) { me->Yell("你居然胆敢挑战我，让你死无葬身之地！", LANG_UNIVERSAL, NULL); }
							if (t == 14745) { me->Yell("现在是时候付出代价了！", LANG_UNIVERSAL, NULL); }
							if (t == 14754) { me->Yell("请打败我吧，无敌是多么寂寞...", LANG_UNIVERSAL, NULL); }
							if (t == 14755) { me->Yell("你的力量将为我带来无尽喜悦...", LANG_UNIVERSAL, NULL); }

							if (t == 14760) { me->Yell("LOL...", LANG_UNIVERSAL, NULL); }
							...
							if (t == 14773) { me->Yell("弱肉强食！", LANG_UNIVERSAL, NULL); }
							if (t == 14792) { me->Yell("我来此只是为了被挑战...", LANG_UNIVERSAL, NULL); }

							//if (t==14799) { me->Yell("Come to me, crusaders. I will remake you!",LANG_UNIVERSAL,NULL); }
							if (t == 14802) { me->Yell("即使是灵魂，也要向我低头！", LANG_UNIVERSAL, NULL); }
							if (t == 14766) { me->Yell("你死定了...", LANG_UNIVERSAL, NULL); }
				...
				case EVENT_MIRROR_IMAGE_MAINSPELL:
					for (uint8 i = 0; i < 2; ++i)
					{
						...
						Clonetarget = SelectTarget(SelectTargetMethod::MaxThreat, 0);
				...
				case EVENT_FRENZY:
					...
					if (t == 14808) { me->Yell("我要你致歉！！！", LANG_UNIVERSAL, NULL); }
					if (t == 14800) { me->Yell("颤抖吧，凡人！", LANG_UNIVERSAL, NULL); }
					if (t == 14787) { me->Yell("痛苦，折磨！", LANG_UNIVERSAL, NULL); }
