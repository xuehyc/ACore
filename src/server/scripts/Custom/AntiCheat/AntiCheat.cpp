...
//		CharacterDatabasePreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_AC);
//		stmt->SetData(0, player->GetName());
//		stmt->SetData(1, player->GetSession()->GetAccountId());
//		stmt->SetData(2, text);
//		stmt->SetData(3, GetTimeString());
//		trans->Append(stmt);
...
//	FixtimeBGMap.clear();
//	return;
//	QueryResult result = WorldDatabase.Query("SELECT id, events from xy_battleground_reward");
//	if (result)
//	{
//		do
//		{
...
//			
//            BattlegroundTypeId bgt = BattlegroundTypeId(fields[0].Get<uint32>());
//			std::vector<uint32> events;
//            Tokenizer data(fields[1].Get<std::string>(), ' ');
