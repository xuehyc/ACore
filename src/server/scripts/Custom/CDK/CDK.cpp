...
void CDKC::AddGossip(Player* player, Object* obj)
{
	...
	AddGossipItemFor(player,GOSSIP_ICON_BATTLE, "", 9991, 9991, "", 0, true);
...
bool CDKC::Redeem(Player* player, uint32 sender, uint32 action, std::string cdk)
{
	if (sender == 9991 && action == 9991)
	{
		...
		if (itr != CDKMap.end())
		{
			...
			WorldDatabase.Execute(sWorld->getBoolConfig(CONFIG_ZHCN_DB) ? "DELETE FROM __兑换码 WHERE 兑换码 = '%s'" : "DELETE FROM _CDK WHERE cdk = '%s'", cdk);
