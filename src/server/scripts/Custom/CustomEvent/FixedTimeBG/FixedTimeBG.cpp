...
void FixedTimeBG::Load()
{
	...
	do
	{
		...
		FixedTimeBGTemp.winRewardTemplateId		= fields[1].Get<uint32>();
		FixedTimeBGTemp.loseRewardTemplateId	= fields[2].Get<uint32>();
		FixedTimeBGTemp.killsForEnd				= fields[3].Get<uint32>();
		FixedTimeBGTemp.killsForReward			= fields[4].Get<uint32>();
		FixedTimeBGTemp.killedsForReward		= fields[5].Get<uint32>();
		FixedTimeBGTemp.damageForReward			= fields[6].Get<uint32>();
		FixedTimeBGTemp.healForReward			= fields[7].Get<uint32>();
		FixedTimeBGTemp.resourcesMax			= fields[8].Get<uint32>();
        FixedTimeBGTemp.crossFaction = fields[9].Get<bool>();
		FixedTimeBGTemp.limitHP					= fields[10].Get<uint32>();
		FixedTimeBGTemp.RandomBuffInterval		= fields[11].Get<uint32>();

		for (size_t i = 0; i < 5; i++)
			FixedTimeBGTemp.randomBuffs[i]		= fields[12 + i].Get<uint32>();

        FixedTimeBGTemp.bgName = fields[17].Get<std::string>();//CString
