...
//#include "../acelite/ace/INET_Addr.h"
#include "Player.h"
#include "Pet.h"
#include "LFG.h"
#include "GroupMgr.h"
...
public:
    ...
    void SetPlayerMsg(Player * player, std::string msg) { if (!player) return; uint32 guid = player->GetGUID().GetCounter(); _SoulStoneExPlayerMsgMap[guid] = msg; }
    std::string GetPlayerMsg(Player * player) { if (!player) return ""; uint32 guid = player->GetGUID().GetCounter(); return _SoulStoneExPlayerMsgMap[guid]; }
