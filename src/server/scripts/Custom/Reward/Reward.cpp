...
void Reward::Load()
{
	...
	do
	{
        ...
        uint32 rewId = fields[0].Get<uint32>();
		RewTemplate RewTemp;
		RewTemp.goldCount			= fields[1].Get<uint32>() * GOLD;
		RewTemp.tokenCount			= fields[2].Get<uint32>();
		RewTemp.xp					= fields[3].Get<uint32>();
		RewTemp.hrPoints			= fields[4].Get<uint32>();
		RewTemp.arenaPoints			= fields[5].Get<uint32>();
		RewTemp.statPoints			= fields[6].Get<uint32>();

		Tokenizer spellData(fields[7].Get<std::string>(), '#');
		...
		Tokenizer commandData(fields[8].Get<std::string>(), '#');
		...
		{
			uint32 entry = fields[9 + 2 * i].Get<uint32>();
			uint32 count = fields[10 + 2 * i].Get<uint32>();
...
void Reward::LoadDamCreToSend()
{
	...
	QueryResult itemup = WorldDatabase.Query("select 生物编号,最低伤害量,排行奖励,排行弹窗 from _生物_伤害奖励");
	if (itemup)
	{
		int nCount = 0;
		do
		{
			...
			tmpItem.creatureid = itemup->Fetch()[0].Get<uint32>();
			tmpItem.mindamage = itemup->Fetch()[1].Get<uint32>();
			Tokenizer senditems(itemup->Fetch()[2].Get<std::string>(), '#');
			tmpItem.sendgossipcount = itemup->Fetch()[3].Get<uint32>();
