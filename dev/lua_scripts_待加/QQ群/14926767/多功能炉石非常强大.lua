	...
	if(player:IsInCombat())then
		...
		if(nowTime>lastTime)then
			...
			if(map)then
				...
				if(NPC)then
					player:SendAreaTriggerMessage("召唤大师成功")
					...
					player:SendAreaTriggerMessage("召唤大师失败")
				end
			end
		else
			player:SendAreaTriggerMessage("召唤大师不能太频繁")
		end
	end
	ST[guid]=lastTime
end

function ST.SummonGNPC(player) --召唤修理训练师
	ST.SummonNPC(player, ST.NPCID1)
end

function ST.SummonENPC(player) --幻化 召唤新手导师
	ST.SummonNPC(player, ST.NPCID2)
end
function ST.SummonNPC_400300(player)  --召唤幻化大师
	ST.SummonNPC(player, 400300)
end
...
local Menu={
	[MMENU]={--主菜单
		...
		{FUNC, "传回位置", 	Stone.GoHome,	GOSSIP_ICON_TAXI,		   false,"是否穿越回|cFFF0F000记录位置|r ?"},
		{FUNC, "记录位置", 	Stone.SetHome,	GOSSIP_ICON_TAXI,          false,"是否记录当前|cFFF0F000位置|r ?"},
		--{FUNC, "修理装备",	   	Stone.RepairAll,	GOSSIP_ICON_MONEY_BAG,	  false,"需要花费金币修理装备 ?"},
		{FUNC, "召唤修理", 	ST.SummonGNPC,	GOSSIP_ICON_TRAINER},
		{FUNC, "召唤幻化",	ST.SummonENPC,	GOSSIP_ICON_TRAINER},
	...
	[MMENU+0x10]={--其他功能
		...
		{MENU, "传送到职业训练师",    MMENU+0x20,GOSSIP_ICON_TAXI},
		{MENU, "传送到商业训练师",    MMENU+0x30,GOSSIP_ICON_TAXI},
	...
	[TPMENU+0x10]={--主要城市
	 	{TP, "[诺森德]|cFFB22222达拉然|r", 571, 5828.42, 471.85, 658.78, 4.208127},
	...
	[TPMENU+0x50]={--诺森德
		...
		{TP, "晶歌森林", 571, 5411.17, -966.37, 167.082, 1.57167},
